...
`/app` is the assistant: one conversation column with a composer (Enter sends, Shift+Enter for a new line, push-to-talk when voice is configured), the assistant's message, its validated cards, and up to four suggestion chips that each post a real suggestion turn (View memories opens the memory sheet). History reloads from `GET /api/turns` for the browser's conversation id; New conversation starts a fresh id so recall can be shown. Failures render as a plain sentence with Retry (same `clientRequestId`, so a request that actually finished returns its stored answer) and Dismiss. The memory sheet lists every memory with its source quote and expiry, and offers Done / Restocked and Delete with a named confirmation. Approval cards call the action routes: Allow, Change (inline title/date/time form, PATCH bumps the version), Cancel, Check status; a succeeded action re-renders as the live confirmation card only when the server returned an event id and link.
