 ...
 * to disk. Requires NEXT_PUBLIC_SUPABASE_URL and the server key in .env.
 */
...
if (!url || !serviceKey) {
  console.error("Missing NEXT_PUBLIC_SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY/SUPABASE_SECRET_KEY in .env");
  process.exit(1);
}
if (!email || !password) {
...
// Write DEMO_USER_ID into .env without touching anything else.
const envPath = ".env";
